...
function Friends({route, navigation}){
  const {refresh, openRequests} = route.params;
  return(
    <FriendsList refresh={refresh} openRequests={openRequests}  navigation={navigation} onDrawerPress={() => navigation.openDrawer()} ></FriendsList>
  );
}
